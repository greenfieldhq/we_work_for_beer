defmodule WeWorkForBeer.LocationSeeder do
  ...
  def seed do
    for location_attrs <- _locations_attrs do
      location = _create_or_update_location(location_attrs)

      for floor_attrs <- location_attrs.floors do
        _find_or_create_floor(Map.put(floor_attrs, :location_id, location.id))
      end
    end
  end
  ...
  defp _create_or_update_location(attrs) do
    ...
    if location do
      ...
      location = _create_location(attrs)
  ...
  defp _find_or_create_floor(attrs) do
    floor = Repo.get_by(Floor, name: attrs.name, location_id: attrs.location_id)

    if floor do
      IO.puts "#{floor.name} already exists"
    else
      floor = _create_floor(attrs)
      IO.puts "#{floor.name} created"
    end

    floor
  end

  def _locations_attrs do
    [
      %{
        city: "Amsterdam",
